...
### Requirement: Greeting without drawing changes

When `CADLENS` runs, the system SHALL print a greeting containing `CAD Lens` on the AutoCAD command line, finish without further user input, and leave the open DWG unchanged.

#### Scenario: Command runs with a drawing open

- **WHEN** the user runs `CADLENS` with a DWG open
- **THEN** a greeting containing `CAD Lens` appears on the command line, the command finishes, and the DWG remains unchanged