...
Checked tasks have supporting source, automated, or user-reported host evidence. Unchecked tasks may already be implemented but still include unverified acceptance cases. The user confirmed navigation/cleanup, full-row clicks, and shared highlighting/clearing in two visible viewports. The general report that all checks passed is retained, without inventing a detailed host/version or fixture matrix. See verification.md for the current summary and historical records.
...
- [x] 1.5 Add `Microsoft.Extensions.DependencyInjection` with an exact compatible version and conditional host reference in `Directory.Build.props`; establish the AutoCAD composition root with scope/build validation and an explorer-scope factory, then verify restore and provider validation, rejection of singleton-to-scoped dependencies, and no container dependency in Core/Lenses/UI. Extend graph validation as concrete implementations are registered.
  - September 20: added five managed composition tests for missing registrations, singleton/scoped validation, root resolution, fresh session scopes/disposal, and assembly boundaries. Successful user host runs now also demonstrate native adapter construction and resolution of the panel graph; provider scope/build validation is enabled.

## 2. Early AutoCAD integration checks

- [x] 2.1 Implement one queued host task service with application-context execution, a document check and lock, cancellation, and native-callback exception containment; verify queue behavior with controllable native callbacks and execute a read-only request from a modeless callback in AutoCAD.
...
- [ ] 2.5 Verify shared highlighting/dimming across Model and adjacent layout viewports wherever inventory objects are visible, per-viewport hidden-state preservation, paper-space inventory boundaries, and cleanup when switching documents, including coexistence with another overrule; record host/version, property comparisons, and cleanup results. Treat unmet dimming, visibility, or document-boundary requirements as a blocker to full explorer integration, not as permission to ship highlighting alone.
...
- [ ] 4.2 Connect generic emphasis requests to the verified host visualization adapter; verify layer-to-type-to-object navigation narrows emphasis, Back expands it correctly, and excluding the selected layer clears its effects and returns to the list.
  - Navigation now sends explicit targets through the existing highlight queue for the user-requested trial. Managed tests cover narrowing, ancestor/root restoration, filter/refresh cleanup, and cancellation. The user confirmed the navigation/cleanup trial. The user also confirmed shared highlighting and clearing in two visible viewports after the all-viewport regeneration fix. The broader adapter acceptance matrix in 2.3-2.5 remains open.
...
- [ ] 6.1 Wire the AutoCAD-created `CADLENS` entry point through the composition owner to resolve the constructor-injected Layers/session/UI graph from an explorer scope without printing a greeting; verify the complete graph validates, first invocation returns control without command-line input, repeated invocation reuses one panel and scope, and reopening creates fresh scoped instances with both toggles reset.
- [ ] 6.2 Handle document activation/destruction and space/viewport transitions with UI reset and refresh while retaining the explorer DI scope and replacing document-bound state; verify switches release old subscriptions/effects, scoped adapters do not retain the opening document, stale requests cannot publish or act on another document, and closing the last drawing disables drawing-dependent actions.
  - Automatic first load and context refresh are implemented. Subscriptions follow the active document; no-drawing command availability and retained filter options have managed tests. The user gave overall acceptance of the checks they ran; specific document/layout switching and shutdown outcomes were not separately recorded and remain open.
- [ ] 6.3 Coalesce database changes at safe command boundaries and revalidate live action targets; verify edit, erase, undo/redo, layer rename/status changes, and a queued action against a deleted object refresh results or restore the nearest valid ancestor without stale highlights.
  - Database append/modify/erase and undo/redo notifications now coalesce into an idle refresh through the existing queue. Reads wait for a quiescent editor and an available panel command. The user later reported that all checks passed after the edit/erase/Undo/Redo/rename test was requested. Treat this as general user acceptance; detailed results and the queued-deleted-target case were not separately recorded.
